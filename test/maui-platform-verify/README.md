...
The 138-check MAUI-on-OpenHarmony interaction harness. It builds the platform slice sources from the
...
and the accessibility shadow tree snapshot.
...
dotnet build -v:q
dotnet bin/Debug/net11.0/verify.dll | grep -c '\[verify\]'   # expect 138
```
...
- The suite fails loudly (unhandled exception) when a slice change breaks startup; keep it at
  138 checks when touching the platform slice.
