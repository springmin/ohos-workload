...
#                    expectations - 254744 for the ui/shell shell, 18532 for the headless shell
...
# Current abc size expectations: the ui/shell ArkTS shell (254744 B) and the headless shell
...
EXPECT_ABC="${KIT_EXPECTED_ABC:-254744,18532}"
...
Without an argument the current directory is used (it must contain SHA256SUMS).
  ...
  --expected-abc <bytes[,bytes]>
                        abc size expectation (default: 254744,18532 = the ui/shell and the
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --expected-abc)
            shift
            [ $# -gt 0 ] || { warn "--expected-abc 需要逗号分隔的字节数（如 254744,18532）"; usage >&2; exit 2; }
            EXPECT_ABC="$1"
            EXPECT_ABC_PINNED=1
            ;;
        --host-deps)
...
for _abc in $EXPECT_ABC; do
    case "$_abc" in
        ''|*[!0-9]*) warn "--expected-abc 需要逗号/空格分隔的字节数（如 254744,18532），得到: $EXPECT_ABC"; usage >&2; exit 2 ;;
    esac
done
