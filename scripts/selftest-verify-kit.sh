...
#                 assertion passes (index 1588 B, abc 254744 B / PANDA 13.0.1.0, 14 .so,
...
#   S5 abcdrift   abc 214000 B -> WARN + KIT OK by default; --expected-abc 254744 -> FAIL;
...
assert_contains "S1 abc 254744 / PANDA 13.0.1.0" "ets/modules.abc 254744 B，PANDA 头版本 13.0.1.0" "$LOG_FILE"
...
assert_contains "S5 warns about the drifted size" "abc 大小 214000 不是当前期望（254744/18532）" "$LOG_FILE"
assert_contains "S5 KIT OK carries the WARN count" "KIT OK（5 条 WARN" "$LOG_FILE"
run_verify "$K" --expected-abc 254744
assert_rc 1 "$RC" "S5 pinned --expected-abc 254744 turns the drift into a FAIL"
assert_contains "S5 reports the pinned set" "不在 --expected-abc 254744 内" "$LOG_FILE"
